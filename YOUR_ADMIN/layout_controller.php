...
//-bof-zca_diy_tpl  *** 2 of 10 ***
      $layout_box_status_mobile = zen_db_prepare_input($_POST['layout_box_status_mobile']);
      $layout_box_status_tablet = zen_db_prepare_input($_POST['layout_box_status_tablet']);
//-eof-zca_diy_tpl  *** 2 of 10 ***		
//-bof-zca_diy_tpl  *** 3 of 10 ***	
      $db->Execute("UPDATE " . TABLE_LAYOUT_BOXES . "
                    SET layout_box_status = " . zen_db_input($layout_box_status) . ",
                        layout_box_location = " . zen_db_input($layout_box_location) . ",
                        layout_box_sort_order = " . zen_db_input($layout_box_sort_order) . ",
                        layout_box_sort_order_single = " . zen_db_input($layout_box_sort_order_single) . ",
                        layout_box_status_single = " . zen_db_input($layout_box_status_single) . ",
                        layout_box_status_mobile = " . zen_db_input($layout_box_status_mobile) . ",
                        layout_box_status_tablet = " . zen_db_input($layout_box_status_tablet) . "
                    WHERE layout_id = " . zen_db_input($box_id));
//-eof-zca_diy_tpl  *** 3 of 10 ***	
...
//-bof-zca_diy_tpl  *** 4 of 10 ***				
          $db->Execute("UPDATE " . TABLE_LAYOUT_BOXES . "
                        SET layout_box_status = " . $reset_box['layout_box_status'] . ",
                            ...
                            layout_box_status_mobile = " . $reset_box['layout_box_status_mobile'] . ",
                            layout_box_status_tablet = " . $reset_box['layout_box_status_tablet'] . "
                        WHERE layout_box_name = '" . $reset_box['layout_box_name'] . "'
                        AND layout_template='" . zen_db_input($template_dir) . "'");
//-eof-zca_diy_tpl  *** 4 of 10 ***												
        }
...
<?php //-bof-zca_diy_tpl  *** 5 of 10 *** ?>
                <th class="dataTableHeadingContent text-center"><?php echo 'MOBILE Status'; ?></th>
                <th class="dataTableHeadingContent text-center"><?php echo 'TABLET Status'; ?></th>								
<?php //-eof-zca_diy_tpl  *** 5 of 10 *** ?>							
            ...
            <tbody>
              ...
              <?php
//-bof-zca_diy_tpl  *** 5 of 10 ***
              $column_controller = $db->Execute("SELECT layout_id, layout_box_name, layout_box_status, layout_box_location, layout_box_sort_order, layout_box_sort_order_single, layout_box_status_single, layout_box_status_mobile, layout_box_status_tablet
                                                 FROM " . TABLE_LAYOUT_BOXES . "
...
//  if ( (is_object($bInfo)) && ($column_controller->fields['layout_id'] == $bInfo->layout_id) ) {
                if (isset($bInfo) && is_object($bInfo) && ($column_controller->fields['layout_id'] == $bInfo->layout_id)) {
                  echo '              <tr class="dataTableRowSelected" onclick="document.location.href=\'' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, 'page=' . $_GET['page'] . '&cID=' . $bInfo->layout_id . '&action=edit') . '\'" role="button">' . "\n";
                } else {
                  echo '              <tr class="dataTableRow" onclick="document.location.href=\'' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, 'page=' . $_GET['page'] . '&cID=' . $column_controller->fields['layout_id']) . '\'" role="button">' . "\n";
                }
                ?>
...
<?php //-bof-zca_diy_tpl  *** 6 of 10 *** ?>
              <td class="<?php echo ( (file_exists($boxes_directory . $column_controller->fields['layout_box_name']) || file_exists($boxes_directory_template . $column_controller->fields['layout_box_name'])) ? 'dataTableContent' : 'messageStackError' ); ?>" align="center"><?php echo ($column_controller->fields['layout_box_status_mobile'] == '1' ? TEXT_ON : '<span class="alert">' . TEXT_OFF . '</span>'); ?></td>
              <td class="<?php echo ( (file_exists($boxes_directory . $column_controller->fields['layout_box_name']) || file_exists($boxes_directory_template . $column_controller->fields['layout_box_name'])) ? 'dataTableContent' : 'messageStackError' ); ?>" align="center"><?php echo ($column_controller->fields['layout_box_status_tablet'] == '1' ? TEXT_ON : '<span class="alert">' . TEXT_OFF . '</span>'); ?></td>
<?php //-eof-zca_diy_tpl  *** 6 of 10 *** ?>
...
//-bof-zca_diy_tpl  *** 7 of 10 ***
            switch ($bInfo->layout_box_status_mobile) {
              case '0': $layout_box_status_mobile_on = false;
                $layout_box_status_mobile_off = true;
                break;
              case '1':
              default: $layout_box_status_mobile_on = true;
                $layout_box_status_mobile_off = false;
            }
            switch ($bInfo->layout_box_status_tablet) {
              case '0': $layout_box_status_tablet_on = false;
                $layout_box_status_tablet_off = true;
                break;
              case '1':
              default: $layout_box_status_tablet_on = true;
                $layout_box_status_tablet_off = false;
            }
//-eof-zca_diy_tpl  *** 7 of 10 ***
...
//-bof-zca_diy_tpl  *** 8 of 10 ***
                switch ($bInfo->layout_box_status_mobile) {
                  case '0': $in_status_mobile = false;
                    $out_status_mobile = true;
                    break;
                  case '1': $in_status_mobile = true;
                    $out_status_mobile = false;
                    break;
                  default: $in_status_mobile = true;
                    $out_status_mobile = false;
                }
                switch ($bInfo->layout_box_status_tablet) {
                  case '0': $in_status_tablet = false;
                    $out_status_tablet = true;
                    break;
                  case '1': $in_status_tablet = true;
                    $out_status_tablet = false;
                    break;
                  default: $in_status_tablet = true;
                    $out_status_tablet = false;
                }								
//-eof-zca_diy_tpl  *** 8 of 10 ***

                $heading[] = array('text' => '<h4>' . TEXT_INFO_HEADING_EDIT_BOX . '</h4>');

                $contents = array('form' => zen_draw_form('column_controller', FILENAME_LAYOUT_CONTROLLER, 'page=' . $_GET['page'] . '&cID=' . $bInfo->layout_id . '&action=save' . '&layout_box_name=' . $bInfo->layout_box_name, 'post', 'class="form-horizontal"'));
...
//-bof-zca_diy_tpl  *** 9 of 10 ***
                $contents[] = array('text' => '<br>' . zen_draw_label('Mobile Display Status', 'layout_box_status_mobile' , 'class="control-label"') . '<div class="radio"><label>' . zen_draw_radio_field('layout_box_status_mobile', '1', $in_status_mobile) . TEXT_ON . '</label></div><div class="radio"><label>' . zen_draw_radio_field('layout_box_status_mobile', '0', $out_status_mobile) . TEXT_OFF . '</label></div>');
                $contents[] = array('text' => '<br>' . zen_draw_label('Tablet Display Status', 'layout_box_status_tablet' , 'class="control-label"') . '<div class="radio"><label>' . zen_draw_radio_field('layout_box_status_tablet', '1', $in_status_tablet) . TEXT_ON . '</label></div><div class="radio"><label>' . zen_draw_radio_field('layout_box_status_tablet', '0', $out_status_tablet) . TEXT_OFF . '</label></div>');
//-eof-zca_diy_tpl  *** 9 of 10 ***               
								$contents[] = array('align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-primary">' . IMAGE_UPDATE . '</button> <a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, 'page=' . $_GET['page'] . '&cID=' . $bInfo->layout_id . '&layout_box_name=' . $bInfo->layout_box_name) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>');
                break;
              case 'delete':
                ...
                $contents[] = array('align' => 'text-center', 'text' => '<br><button type="submit" class="btn btn-danger">' . IMAGE_UPDATE . '</button> <a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, 'page=' . $_GET['page'] . '&cID=' . $bInfo->layout_id) . '" class="btn btn-default" role="button">' . IMAGE_CANCEL . '</a>');
                break;
              default:
                if (is_object($bInfo)) {
                  $heading[] = array('text' => '<h4>' . TEXT_INFO_LAYOUT_BOX . $bInfo->layout_box_name . '</h4>');
                  $contents[] = array('text' => '<a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, 'page=' . $_GET['page'] . '&cID=' . $bInfo->layout_id . '&action=edit') . '" class="btn btn-primary" role="button">' . IMAGE_EDIT . '</a>');
...
//-bof-zca_diy_tpl  *** 10 of 10 ***
                  $contents[] = array('text' => 'Mobile Status Display' . ' ' . ($bInfo->layout_box_status_mobile == '1' ? TEXT_ON : TEXT_OFF));
                  $contents[] = array('text' => 'Tablet Status Display' . ' ' . ($bInfo->layout_box_status_tablet == '1' ? TEXT_ON : TEXT_OFF));
//-eof-zca_diy_tpl  *** 10 of 10 ***
                  if (!(file_exists($boxes_directory . $bInfo->layout_box_name) || file_exists($boxes_directory_template . $bInfo->layout_box_name))) {
                    ...
                    $contents[] = array('text' => '<a href="' . zen_href_link(FILENAME_LAYOUT_CONTROLLER, 'page=' . $_GET['page'] . '&cID=' . $bInfo->layout_id . '&action=delete' . '&layout_box_name=' . $bInfo->layout_box_name) . '" class="btn btn-warning" role="button">' . IMAGE_DELETE . '</a>');
                  }
                }
                break;
            }
