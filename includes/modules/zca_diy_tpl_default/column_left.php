...
$column_left_display= $db->Execute("select layout_box_name, layout_box_status_mobile, layout_box_status_tablet from " . TABLE_LAYOUT_BOXES . " where layout_box_location = 0 and layout_box_status= '1' and layout_template ='" . $template_dir . "'" . ' order by layout_box_sort_order');
// safety row stop
$box_cnt=0;
...
if ( $column_left_display->fields['layout_box_status_mobile'] == '0' && $_SESSION['layoutType'] != 'full' ) {
    $flag_disable_box = true;
  } else {
    $flag_disable_box = false;
 }
} else if ( $detect->isTablet() || $_SESSION['layoutType'] == 'tablet'){
if ( $column_left_display->fields['layout_box_status_tablet'] == '0' && $_SESSION['layoutType'] != 'full' ) {
    $flag_disable_box = true;
  } else {
    $flag_disable_box = false;
 }
