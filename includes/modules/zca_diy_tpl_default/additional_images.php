...
$col_per_row = IMAGES_AUTO_ADDED;
} else if ( $detect->isMobile() && !$detect->isTablet() || $_SESSION['layoutType'] == 'mobile' ) {
$col_per_row = IMAGES_AUTO_ADDED_MOBILE;
} else if ( $detect->isTablet() || $_SESSION['layoutType'] == 'tablet'){
$col_per_row = IMAGES_AUTO_ADDED_TABLET;
} else {
$col_per_row = IMAGES_AUTO_ADDED;
}
...
if ($num_images > 0) {
    $row = 0;
    $col = 0;
    if ($num_images < $col_per_row || $col_per_row == 0 ) {
...
//-bof-zca_diy_tpl_demo  *** 1 of 0 ***
        ...
        $link_parameters = 'class="additionalImages centeredContent columns' . $col_per_row . '"';
